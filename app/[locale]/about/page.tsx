...
export default async function AboutPage({ params }: LocalePageProps) {
  ...
  const heroPhoto = photos.find((photo) => photo.id === "interior-001") ?? photos.find((photo) => photo.category === "interior");

  return (
    <>
      <BreadcrumbJsonLd locale={lang} path="about" label={d.about.title} />
      <PageHero
        locale={lang}
        variant="about"
        eyebrow={d.about.heroEyebrow}
        title={d.about.heroTitle}
        intro={d.about.heroIntro}
        photo={heroPhoto}
      />
      <section className="section">
        <div className="container about-grid">
          {stories.map((story, index) => (
            <MotionReveal className="about-card" delay={index * .08} key={story.title}>
      ...
      <section className="about-statement">
        <div className="container">
          ...
          <p>{lang === "ja" ? "昼のひと皿から、仲間と囲む夜のテーブルまで。" : "From a midday plate to an evening table shared with friends."}</p>
        </div>
      </section>
    </>
  );
